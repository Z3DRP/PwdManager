...
const flashAlert = document.getElementById('falert');
...
function showAlert() {
    ...
    falert.style.transform = 'translateX(0)';
    alertTimeout = setTimeout(() => {
        falert.style.transform = 'translateX(-400px)';
    }, 4000);
}
...
function main(toastData, hasAlert) {
    if (toastData !== undefined) {
        //showToast(toastData);
        if (toastData['hasAlert']) {
            showAlert();
        }
    }
}
